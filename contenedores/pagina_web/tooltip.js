...
function crearTooltips(nombreYcods) {
    ...
    for (const key in nombreYcods) {
        ...
        const options = {
            ...
            url: `https://api.euskadi.eus/euskalmet/weather/regions/basque_country/zones/${nombreYcods[key]}/locations/${key.toLowerCase()}/forecast/at/${fechaActualSeparada[0]}/${fechaActualSeparada[1]}/${fechaActualSeparada[2]}/for/${fechaMananaSeparada[0]}${fechaMananaSeparada[1]}${fechaMananaSeparada[2]}`,
            timeout: 0,
            headers: {
            ...
            $.ajax(options).done(function (response) {
                ...
                tarjeta.title = response["forecastText"]["SPANISH"]
              }).fail(function(jqXHR, textStatus, errorThrown) {
                console.error(`Error en la ciudad ${key}: ${textStatus}`, errorThrown);
            });
    }
}
